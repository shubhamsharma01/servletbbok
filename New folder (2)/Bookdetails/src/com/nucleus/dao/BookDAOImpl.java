...
public class BookDAOImpl implements BookDAO {
	...
	public void saveBook(Book book) {
		...
		String publication=book.getPublication();
				...
				try {
			pstmt=con.prepareStatement("insert into BookWebApp1 values(?,?,?,?,?,?)");
	...
	public Book getBookByBookId(String bookId) {
		try {
			PreparedStatement pstmt=con.prepareStatement("select * from BookWebApp1 where bookId=?");
	...
	public void deleteBookByBookId(String bookId) {
           try {
               PreparedStatement pstmt=con.prepareStatement("delete from BookWebApp1 where bookId=?");
			pstmt.setString(1, bookId);
			pstmt.executeQuery();
		} catch (SQLException e) {
